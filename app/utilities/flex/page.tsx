...
export default function FlexOverviewPage() {
    ...
    <>
      <div className="max-w-7xl mx-auto px-4 py-12 space-y-8">
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="border border-border rounded-lg p-4 bg-card/30">
              ...
              <div className="grid md:grid-cols-4 gap-3 mb-3">
                ...
                {/* wrap */}
                <div>
                  <div className="text-xs text-muted-foreground mb-1">Wrap</div>
        ...
        <InteractiveChallenge
          title="The Squished Menu"
          description="This navigation menu is broken on mobile screens. The logo, links, and button are all trying to fit in a single horizontal row (`flex-row`), causing them to overflow or squish. Switch the layout to `flex-col` to stack them vertically for a cleaner mobile experience."
          codeSnippet={`<nav class="w-64 bg-slate-900 text-white p-6 rounded-xl border border-slate-700">
  <div class="flex {input} gap-4 items-center">
    ...
    <button class="bg-blue-600 px-4 py-2 rounded text-sm font-bold">
      Login
          ...
          options={["flex-row", "flex-wrap", "block", "flex-col"]}
          correctOption="flex-col"
          renderPreview={(userClass) => (
            <div className="flex items-center justify-center w-full h-full bg-slate-100 dark:bg-slate-950 p-8 rounded-lg">
              <nav className="w-64 bg-slate-900 text-white p-6 rounded-xl border border-slate-700 shadow-2xl overflow-hidden">
                <div
                  className={`flex ${userClass} gap-4 items-center transition-all duration-500`}
                >
                  {/* Logo */}
                  ...
                  <div
                    className={`flex gap-3 text-sm text-slate-400 ${
                      userClass === "flex-col" ? "flex-col w-full" : ""
                    }`}
                  >
                    <div className="bg-slate-800 h-4 w-12 rounded"></div>
                    <div className="bg-slate-800 h-4 w-16 rounded"></div>
                    <div className="bg-slate-800 h-4 w-14 rounded"></div>
                  </div>

                  {/* Button */}
                  <div
                    className={`bg-blue-600 h-8 rounded w-20 ${
                      userClass === "flex-col" ? "w-full" : ""
                    }`}
                  ></div>
                </div>

                {/* Error Overlay for Row */}
                {userClass === "flex-row" && (
                  <div className="absolute inset-0 bg-red-500/10 border-2 border-red-500 rounded-xl pointer-events-none flex items-center justify-center">
  ...
  <main class="flex-1 p-6">Main content</main>
              ...
              description={
                <>
                  Sidebar uses{" "}
                  <code className="bg-slate-700 px-1 rounded">flex-col</code> to
                  stack links. Mark it{" "}
  ...
  <div class="p-3 rounded bg-slate-700/90 text-white">Older</div>
              ...
              description={
                <>
                  ...
                  to show newest visually at top while keeping chronological DOM
                  order. Add{" "}
  ...
  <input class="flex-1 min-w-[120px]" placeholder="Add tag" />
              ...
              description={
                <>
                  ...
                  <code className="bg-slate-700 px-1 rounded">min-w</code> so it
                  doesn't collapse.
                </>
              }
            >
          ...
          <div className="text-sm text-muted-foreground">
            ...
            *-reverse) does not change DOM order — keyboard and screen-reader
            users follow DOM. If visual order must match keyboard order, change
            the DOM or manage focus/ARIA.
          </div>
        </section>
        ...
        <TipsSection
          tips={[
            {
              bold: "Responsive switches:",
              text: "Use md:flex-row/flex-col for mobile-first design patterns",
            },
            {
              bold: "Combine properties:",
              text: "Use basis- with flex-grow for predictable card widths",
            },
            {
              bold: "Wrap handling:",
              text: "Use flex-wrap for tags/galleries with fixed widths to maintain rows",
            },
            {
              bold: "Order testing:",
              text: "Always test keyboard navigation when using *-reverse utilities",
            },
            {
              bold: "Performance:",
              text: "Flexbox is GPU-accelerated - prefer over manual calculations",
            },
          ]}
        />
      </div>
